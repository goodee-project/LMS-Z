		...
		<div class="col-md-7 col-lg-7">
				<div class="card" id="cardStyle">
					<div class="card-body">
						...
						<form id="signupForm" method="post" action="${path}/managerSignup">
							<table class="table table" id="signupTable">
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<input type="text" class="form-control-plaintext form-control border-black" id="managerId" name="managerId">
										<div id="textId">
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<div>
											<input type="password" class="form-control-plaintext form-control border-black" id="managerPw" name="managerPw">
											<div id="textPw">
												영문/숫자/특수문자를 조합하여 8~20자로 입력해주세요.
											</div>                                    			
										</div>
										<div>
											<input type="password" class="form-control-plaintext form-control border-black" id="managerPw2" style="margin-top:15px">
											<div id="textPw2">
												비밀번호를 다시 한번 입력해주세요.
											</div>                                    			
										</div>
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<input type="text" class="form-control-plaintext form-control border-black" id="managerName" name="managerName">
										<div id="textName">
											이름을 입력해주세요.
										</div>
									</td>
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<input type="text" class="form-control-plaintext form-control border-black" id="managerEmail" name="managerEmail">
										<div id="textEmail">
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<select class="form-control-plaintext form-control border-black" id="managerPhone1" name="managerPhone1">
											<option value="">선택</option>
											<option value="010">010</option>
											<option value="011">011</option>
										</select>
										<input type="text" class="form-control-plaintext form-control border-black" id="managerPhone2" name="managerPhone2">
										<input type="text" class="form-control-plaintext form-control border-black" id="managerPhone3" name="managerPhone3">
										<div id="textPhone">
											핸드폰 번호를 입력해주세요.
										</div>
									</td>
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										...
										<input type="radio" class="managerGender" name="managerGender" value="여" style="margin-left:15px;">여
										<div id="textGender">
											성별을 선택해주세요
										</div>
									</td>
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<input type="text" class="form-control-plaintext form-control border-black" id="managerAddressMain" name="managerAddressMain" placeholder="도로명을 입력해 주세요">
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<input type="text" class="form-control-plaintext form-control border-black" id="managerAddressSub" name="managerAddressSub">
								...
								<tr>
									...
									<td class="font-14 font-weight-medium text-dark">
										<input type="date" class="form-control-plaintext form-control border-black" id="managerBirth" name="managerBirth">
										<div id="textBirth">
											생일을 입력해주세요.
										</div>
									</td>
    ...
    <script>
		...
		$('#btnSignup').click(function(){
			...
			if(successId == ''){
				...
				$('#managerId').css('outline', '2px solid #FF0000');
			}

			if(successPw == ''){
				$('#textPw').text('X 영문/숫자/특수문자를 조합하여 8~20자로 입력해주세요.')
							.attr('class', 'text-danger');
				$('#managerPw').css('outline', '2px solid #FF0000');
			}

			if(successPw2 == ''){
				$('#textPw2').text('X 비밀번호를 다시 한번 입력해주세요.')
							 .attr('class', 'text-danger');
				$('#managerPw2').css('outline', '2px solid #FF0000');
			}

			if(successName == ''){
				$('#textName').text('X 이름을 입력해주세요.')
				  			  .attr('class', 'text-danger');
				$('#managerName').css('outline', '2px solid #FF0000');
			}

			if(successEmail == ''){
				$('#textEmail').text('X 이메일을 문자 @을 붙인 형식으로 입력해주세요.')
	   			   			   .attr('class', 'text-danger');
				$('#managerEmail').css('outline', '2px solid #FF0000');
			}

			if(successPhone1 == ''){
				$('#textPhone').text('X 핸드폰 번호를 입력해주세요.')
				  			   .attr('class', 'text-danger');
				$('#managerPhone1').css('outline', '2px solid #FF0000');
			}
			
			if(successPhone2 == ''){
				$('#textPhone').text('X 핸드폰 번호를 입력해주세요.')
				  			   .attr('class', 'text-danger');
				$('#managerPhone2').css('outline', '2px solid #FF0000');
			}
			
			if(successPhone3 == ''){
				$('#textPhone').text('X 핸드폰 번호를 입력해주세요.')
				  			   .attr('class', 'text-danger');
				$('#managerPhone3').css('outline', '2px solid #FF0000');
			}
			...
			if(successAddressMain == ''){
				$('#managerAddressMain').css('outline', '2px solid #FF0000');
			}

			if(successBirth == ''){
				$('#textBirth').text('X 생일을 입력해주세요.')
							   .attr('class', 'text-danger');	
				$('#managerBirth').css('outline', '2px solid #FF0000');
			}
		});
		...
		var checkPw2 = /[~!@#$%^&*()_+|<>?:{}]/;
		// managerPw에 대해서 blur로 감지
		// 만약 비닐번호가 특수문자를 포함하고 규약에 맞다면 Pw 활성화
		$('#managerPw, #managerPw2').blur(function(){
			...
			if(successPw2 == 'success' && $('#managerPw').val() != $('#managerPw2').val()){
		...
		$('#managerPw2').blur(function(){
			if(successPw == ''){
				$('#textPw2').text('X 비밀번호를 먼저 입력해주세요.')
							 .attr('class', 'text-danger');
				$(this).css('outline', '2px solid #FF0000');
				successPw2 = '';
			} else if($('#managerPw').val() == $('#managerPw2').val()){
				$('#textPw2').text('');
				$(this).css('outline', '2px solid #5CD1E5');
				successPw2 = 'success';
			} else{
		...
		// managerPhone을 첫번째를 실시간 입력감지하고 가운데랑 마지막 번호는 blur로 감지
		...
		function addressClick(dataSet){
			...
			$('#addressTable').empty();
			$('#managerAddressMain').css('outline', '2px solid #5CD1E5')
									.prop('readonly', 'true');
		...
		$('#btnAddressReset').click(function(){
			$('#managerAddressMain').removeAttr('style')
						   .prop('readonly', false)
						   .val('');
