...
<body>
	                                ...
	                                <table id="myInfoTable2" class="table table">
										<tr>
											...
											<td class=" font-14 font-weight-medium text-dark">
												<input type="password" id="managerPw" class="form-control-plaintext form-control-lg bg-light">
												<input type="hidden" id="accountId" name="accountId" value="${managerId}">
											</td>
										</tr>
										<tr>
											<th class=" font-14 font-weight-medium text-dark">새 비밀번호</th>
											<td class=" font-14 font-weight-medium text-dark">
												<input type="password" id="managerNewPw" class="form-control-plaintext form-control-lg bg-light" name="accountPw">
												<div id="textNewPw">
										...
										<tr>
											...
											<td class=" font-14 font-weight-medium text-dark">
												<input type="password" class="form-control-plaintext form-control-lg bg-light" id="managerNewPw2">
												<div id="textNewPw2">
													비밀번호를 다시 한번 입력해주세요.
												</div>
											</td>
    ...
    <script>
		var managerPwCheck = '';
		var managerNewPwCheck = '';
		var managerNewPw2Check = '';
		...
		$('#btnResult').click(function(){
			if(managerPwCheck == ''){
			...
			} else if(managerNewPwCheck == ''){
				alert('새 비밀번호를 형식에 맞게 입력해주세요.');
			} else if(managerNewPw2Check == ''){
				...
				$.ajax({
					...
					data:{accountId: $('#accountId').val(), accountPw: $('#managerPw').val()},
		...
		$('#managerPw').blur(function(){
			if($('#managerPw').val().length > 0){
				managerPwCheck = 'check';
			} else{
				managerPwCheck = '';
			}
		});
		...
		$('#managerNewPw').blur(function(){
			if(checkPw.test($('#managerNewPw').val())){
				if(checkPw2.test($('#managerNewPw').val())){
					$('#textNewPw').text('○ 사용 가능한 비밀번호힙니다.')
		   		      			   .attr('class', 'text-primary');
	      			managerNewPwCheck = 'check';
				} else{
					$('#textNewPw').text('X 비밀번호에 특수문자를 추가해주세요.')
		 			   		       .attr('class', 'text-danger');
					managerNewPwCheck = '';
				}
			} else if($('#managerNewPw').val().length < 1){
		...
		$('#managerNewPw2').blur(function(){
			if($('#managerNewPw').val().length < 1){
				$('#textNewPw2').text('X 새 비밀번호를 다시 한번 입력해주세요.')
	   		      	  	 	    .attr('class', 'text-danger');
				managerNewPwCheck = '';
			} else{
				if($('#managerNewPw').val() == $('#managerNewPw2').val()){
					$('#textNewPw2').text('○ 입력한 비밀번호가 일치합니다.')
	      			  			   .attr('class', 'text-primary');
					managerNewPw2Check = 'check';
				} else{
