                            ...
                            <div class="card-body">
                               <h4 class="card-title">과목 목록</h4>
	                            <div>
	                            	<table id="lmsTable" class="table table" style="margin-top: 20px; text-align: center;">
	                            		...
	                            		<tbody>
											<c:forEach var="s" items="${subjectList }">
												<tr>
													<td>${s.subjectNo }</td>
													<td><a class="btn btn-outline-light bg-light text-secondary btn-block" href="${path }/manager/subjectOne/${s.subjectNo}">${s.subjectName }</a></td>
	                            	...
	                            	<a class="btn btn-success" style="border-radius: 3px; float: right;" href="${path }/manager/insertSubject">과목 추가</a>
	                            	<br>
								   	<div id="paging" style="text-align: center; padding: 7px;">
								   		<!-- 첫페이지이고 전체 페이지가 '1'이 아닌 경우 이전버튼 표시 -->
								   		<c:if test="${startPage!=1 && lastPage!=1}">
								    		<span>
								    			<a href="${path}/manager/subjectList/${startPage-10}">이전</a>
								    		</span>
								   		</c:if>
								     	...
								     	<c:forEach var="i" begin="${startPage }" end="${lastPage}">
								     		...
								     		<c:if test="${currentPage != i }">
									      		<span>
									      			<a class="font-18" href="${path}/manager/subjectList/${i}">${i}</a>
									      		</span>
								     		</c:if>
								     	</c:forEach>
								    	</c:if>
								    	...
								    	<c:if test="${startPage+9<lastPage && lastPage != startPage}">
									     	<span>
									     		<a href="${path}/manager/subjectList/${startPage+10}">다음</a>
									     	</span>
								    	</c:if>
								   	</div>
	                            </div>
